...
def loadData():
    ...
    global allData
    allData = pd.read_csv("dataset_24_mushroom.csv")


def findBestGinis(node, currentGini, data, columns):
...
def HoeffdingTree(columns, delta):
    ...
    for i in range(0, int(data.size / len(data.columns))):
        ...
        #print(currentGini)
        if currentGini != 0.0:
            best, second_best = findBestGinis(currentNode, currentGini, data, columns)
            epsilon = Util.epsilon(total, 4, delta) #TODO what is the R?
...
def main(delta):
    ...
    loadData()
    global allData
    allData = allData.iloc[range(0, 3000)]
    ...
    for i in range(0, len(y_test)):
        count += HT.predict(X_test.iloc[i]) == y_test[i][1]
